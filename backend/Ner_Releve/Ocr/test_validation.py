...
# === CONFIG ===
MODEL_PATH = "../Model/" # chemin vers ton modèle entraîné
                       
TEST_DATA_FILE = "test_val/annotation_test.json"  # annotations auto générées
...
# Obtenir le chemin absolu du fichier pour être sûr
full_path = os.path.abspath(TEST_DATA_FILE)
...
for i, item in enumerate(test_data[:30]):  # juste 5 lignes pour aperçu
    doc = nlp(item["text"])
